...
namespace ALUNGAMES
{
    ...
    {
        [SerializeField]
        private CityLayoutConfig cityLayoutConfig;
        ...
        {
            if (cityLayoutConfig == null)
            {
                Debug.LogError("CityLayoutConfig is not assigned!");
                return;
            }

            cityLayouts.Clear();
            foreach (var layout in cityLayoutConfig.CityLayouts)
            {
                string[,] layoutArray = layout.GetLayoutArray();
                if (layoutArray.GetLength(0) > 0 && layoutArray.GetLength(1) > 0)
                {
                    cityLayouts.Add(layoutArray);
                    Debug.Log($"Loaded city layout: {layout.layoutName}");
                }
            }

            if (cityLayouts.Count == 0)
            {
                Debug.LogError("No valid city layouts found in CityLayoutConfig!");
            }
        }
        ...
        private void Awake()
        {
            ...
            if (cityLayoutConfig == null)
            {
                cityLayoutConfig = Resources.Load<CityLayoutConfig>("CityLayoutConfig");
                if (cityLayoutConfig == null)
                {
                    Debug.LogError("Failed to load CityLayoutConfig from Resources folder!");
                    return;
                }
            }
